...
def get_or_create_user(user_id, username, full_name, email=""):
    ...
    if not user:
        ...
        # Update username and full_name only if not set or empty
        c.execute("UPDATE users SET username = COALESCE(username, ?), full_name = COALESCE(full_name, ?) WHERE user_id = ?", (username, full_name, user_id))
        conn.commit()
    conn.close()
...
def generate_summary_image(user_data):
    ...
    # Add title
    draw.text((60, 20), "Bit Mining UK Account Summary", fill=(0, 0, 0), font=title_font)
    ...
    for i in range(len(fields)):
        draw.text((60, y), fields[i], fill=(0, 0, 0), font=font)
        draw.text((200, y), values[i], fill=(0, 128, 0), font=font)  # Green for values
        y += 50
...
async def withdraw_confirm(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if choice in ["No", "Back to Main Menu"]:
        await update.message.reply_text("Back to main menu:", reply_markup=main_menu())
        return ConversationHandler.END
    if choice != "Yes":
    ...
    user_data = get_user_data(user_id)
    await update.message.reply_text(
        f"Your withdrawal request of ${amount:.2f} to wallet {user_data[6]} has been submitted. After blockchain transaction confirmation is complete, you will receive your funds.",
        reply_markup=main_menu()
    )
    await context.bot.send_message(
        chat_id=ADMIN_ID,
...
async def dashboard(update: Update, context: ContextTypes.DEFAULT_TYPE):
    user_data = get_user_data(update.effective_user.id)
    # Add emoji to username dynamically
    username_with_emoji = f"{user_data[1] or 'N/A'} {random.choice(['😎', '🚀', '💡', '🌟'])}" if user_data[1] else 'N/A'
    text = (
        f"Dashboard\n\n"
        f".Full Name: {user_data[2] or 'N/A'}\n"
        f".Username: {username_with_emoji}\n"
...
async def setting(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text("Select a field to update:", reply_markup=setting_menu())
...
def main():
    ...
    application.add_handler(CommandHandler(["set_balance", "set_fullname", "set_email", "set_datejoined", "set_level", "set_status", "set_wallet", "set_username", "set_last_withdraw", "set_last_deposit"], admin_command))
